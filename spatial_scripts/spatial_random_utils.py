...
# FUNCTIONS FOR ALL RANDOM TOPOLOGY

# For evaluation function is median rank
def ranks_for_median_random(my_strategy_factory, iterations=200, max_size=50,
    ...
    for parameter in range(10):
        ...
        # any of the three given topologies
        random.seed(1)
        topology = random.randint(0,2)
        if topology == 0 :
...
def do_table_median_random(table):
    """
...
def score_tables_median_random(tables, pool):
...
def ranks_for_min_random(my_strategy_factory, iterations=200, max_size=50,
...
def do_table_min_random(table):
    ...
    """
    fac = lambda: axelrod.LookerUp(lookup_table=table)
    return(ranks_for_min_random(fac), table)


def score_tables_min_random(tables, pool):
    """Use a multiprocessing Pool to take a bunch of tables and score them,
       based on ranks
    """
