...
# FUNCTIONS FOR COMPLETE TOPOLOGY

# For evaluation function is median rank
def ranks_for_median_complete(my_strategy_factory, iterations=200, max_size=50,
...
def do_table_median_complete(table):
    """
    ...
    return(ranks_for_median_complete(fac), table)


def score_tables_median_complete(tables, pool):
    """Use a multiprocessing Pool to take a bunch of tables and score them,
       based on ranks
    """
    return sorted(pool.map(do_table_median_complete, tables),key=lambda x: x[0], reverse=True)

# For evaluation function is min rank
def ranks_for_min_complete(my_strategy_factory, iterations=200, max_size=50,
...
def do_table_min_complete(table):
    """
    Take a lookup table dict, construct a lambda factory for it, and return
    a tuple of the score and the table itself. The score this time is based
    on ranks
    """
    fac = lambda: axelrod.LookerUp(lookup_table=table)
    return(ranks_for_min_complete(fac), table)


def score_tables_min_complete(tables, pool):
    """Use a multiprocessing Pool to take a bunch of tables and score them,
       based on ranks
    """
    return sorted(pool.map(do_table_min_complete, tables),key=lambda x: x[0], reverse=True)

# for max complete
def ranks_for_max_complete(my_strategy_factory, iterations=200, max_size=50,
...
def do_table_max_complete(table):
    """
    Take a lookup table dict, construct a lambda factory for it, and return
    a tuple of the score and the table itself. The score this time is based
    on ranks
    """
    fac = lambda: axelrod.LookerUp(lookup_table=table)
    return(ranks_for_max_complete(fac), table)


def score_tables_max_complete(tables, pool):
    """Use a multiprocessing Pool to take a bunch of tables and score them,
       based on ranks
    """
    return sorted(pool.map(do_table_max_complete, tables),key=lambda x: x[0], reverse=True)
