...
def ranks_for(my_strategy_factory, max_size, min_size, topology, eval_function,
              repetitions, ub_parameter, set_strategies, max_graph_attempt=500,
              parameter_repetitions=3, turns=40):
    """
    ...
    """
    strategies = choose_strategies(set_strategies)
...
def choose_strategies(set_strategies):
    """A function that returns a list of axelrod players."""
    if set_strategies == 0:
...
def do_table(table, parameters):
    ...
    ub_parameter = parameters[4]
    repetitions = parameters[5]
    set_strategies = parameters[6]
    fac = lambda: axelrod.LookerUp(lookup_table=table)
    return(ranks_for(my_strategy_factory=fac, max_size=max_size,
                     min_size=min_size, set_strategies=set_strategies,
                     eval_function=eval_function, topology=topology,
                     ub_parameter=ub_parameter, repetitions=repetitions)
                     , table)
